# GUIDE COMPLET — Pipeline CI/CD KMP Mobile (v8.4)
...
`README.md` (choix technologiques), `INPUTS.md` (référence des 102 inputs),
...
| **Action** | Un step réutilisable et versionné (comme une fonction). | Nos `emerald-*` (chaque dossier a maintenant son README). |
| **Workflow réutilisable** | Un workflow appelable par un autre (`uses: repo/chemin@version`), avec des `inputs`. | `ci-kmp.yml@v8.4`, appelé par le consumer de chaque app. |
...
| **Secret / Vault** | Valeur sensible jamais affichée. GitHub les masque en `***` dans les logs. | 2 secrets GitHub (ROLEID/SECRETID) qui ouvrent **Vault**, où vivent TOUS les autres secrets. |

### Comment lire le fichier `ci-kmp.yml`

```mermaid
flowchart LR
    A["on: workflow_call\n(inputs: 102 paramètres)"] --> B["jobs:\n16 blocs"]
...
### La chaîne workflow_dispatch → publication (vérifiée en v8.4)
...
Cette chaîne est **testée automatiquement** : `test-branch-rules.yml` (9 cas)
asserte branche par branche l'env/config/distribution ET les flags de
publication, y compris les deux cas dispatch. Un écart = job rouge.

### v8.4 : pourquoi le mécanisme de « label PR » a été retiré
...
(`DgagQaRelease`), le JDK (17 pour Qa, 21 pour Store), la lane Fastlane, le nom
d'artefact, et surtout le **nom du secret Vault de signature** — pré-résolu par
`resolve-context` car les noms Vault sont irréguliers (`match-password-dgag-ent`,
`-fede-public`, `-lp-public` : impossible à déduire d'un simple modèle).
...
### F.1 Signature Android (l'équivalent des « Secure Files » Azure)
...
sequenceDiagram
    ...
    J->>V: lit keystore-<brand>-<env><br/>(keystoreData base64 + 3 mots de passe)
    J->>J: décode base64 → fichier .keystore<br/>au chemin attendu par build.gradle.kts<br/>(chmod 600)
    J->>G: build signé (passwords en env)
    J->>J: cleanup « if: always() » :<br/>keystore supprimé même si le build échoue
```
...
2. **Une seule source de décision** : resolve-context transforme la branche en
   contrat (env/config/distribution/matrices) — parité Azure testée par un
   workflow d'assertions.
...
5. **Tout est versionné et documenté** : workflows par dossier `vX.Y`, actions
