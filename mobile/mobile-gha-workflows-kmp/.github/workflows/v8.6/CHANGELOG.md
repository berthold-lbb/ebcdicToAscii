...
Corrections issues de la revue du premier run réel de signature Prod/Beta +
durcissement de la publication AppLivery.

1. **Signature Android — refonte du flux `vault-keystore`** (`emerald-android-signing`) :
   - **Mot de passe séparé du keystore dans Vault** : l'ancien secret unique
     `keystore-{brandKey}-{env}` (champs `keystoreData`/`keystorePassword`/`keyAlias`/
     `keyPassword`) est éclaté en deux lectures — `keystore-{brandKey}-{env}` (champ
     `value` = keystore base64) et `keystore-{brandKey}-password` (champ `value`,
     store = key password). Le nom du secret password ne dépend plus de l'env.
   - **Alias sorti de Vault** : `keyAlias` n'est plus lu depuis un secret — il vient
     désormais de la configuration des marques (`androidBrandsConfig[].keyAlias`,
     ex. `ajustoreleasekey` / `lapersonnellereleasekey`), via le nouvel input
     `key-alias-literal` (prioritaire sur `key-alias-env`, conservé pour compat).
   - **Écriture sous `$RUNNER_TEMP` (chemin ABSOLU, hors repo)** au lieu du chemin
     relatif du repo : élimine toute ambiguïté de résolution `file()` côté Gradle
     (risque de double `androidApp/androidApp/...`) et évite de laisser un keystore
     décodé dans l'arborescence du repo entre le decode et le cleanup.
   - **Export automatique des variables Gradle** : 4 nouveaux inputs
     (`gradle-keystore-file-env`, `gradle-store-password-env`, `gradle-key-alias-env`,
     `gradle-key-password-env`) permettent à l'action d'écrire directement dans
     `$GITHUB_ENV` les variables nommées `Keystore<Brand>File` /
     `KeystoreStorePassword<Brand>` / `KeystoreAlias<Brand>` /
     `KeystoreKeyPassword<Brand>` lues par `androidApp/build.gradle.kts` — plus besoin
     de les recomposer manuellement dans le workflow.
   - `androidBrandsConfig` (defaults) gagne le champ `keyAlias` pour dgag et tpic.

2. **`emerald-mobile-publish-applivery` — `extra-tags` déprécié, remplacé par `test-run`** :
   - Nouvel input booléen `test-run` (défaut `false`). **Publications réelles**
     (`test-run=false` : main/rc/applivery-archives) → tags **strictement identiques
     à Azure** (`applivery-tags` + `branch:<sha256[0:8]>`), **aucun** tag
     d'identification ajouté — zéro régression sur les canaux réels.
   - **Runs de test** (`test-run=true` : PR/manuel/feature) → tags enrichis
     `test:true`, `run:<n>`, `ref:<branche>` pour filtrer facilement ces builds dans
     AppLivery, sans polluer les publications réelles.
   - `extra-tags` (ancien mécanisme) reste supporté pour compat mais n'est plus
     alimenté par `ci-kmp.yml` — remplacé par `test-run` calculé depuis
     `resolve-context.outputs.branch_type` (`manual`/`pr`/`feature`), pour Android
     et iOS.
   - **Résumé enrichi** (`$GITHUB_STEP_SUMMARY`) : tableau des binaires publiés
     (nom, version, taille, build AppLivery) dans les deux modes (`fastlane`/`api`).
   - **Upload API résilient** : remplacement du simple `curl -sSf` par une version
     avec `--http1.1`, `--connect-timeout`, `--max-time 1800`, `--retry 4`,
     `--retry-all-errors`, capture du corps de réponse et validation explicite du
     code HTTP — corrige les échecs intermittents (524 passerelle, `PROTOCOL_ERROR`
     HTTP/2) observés sur les gros IPA.

3. **iOS — publication AppLivery sans rebuild, fiabilisée** :
   - Nouveau `iosApp/Gemfile.publish` : environnement bundler **minimal**
     (fastlane + plugin applivery uniquement), sans cocoapods/arkana/xcode-install
     — ces gems ont des extensions natives sans binaire précompilé pour Linux et
     faisaient échouer `bundle install` sur le runner Linux du job de publication.
   - `iosApp/fastlane/Fastfile` : ajout de `gym_export_method` (traduit les types
     de profil `match` — `appstore`/`adhoc`/… — vers les valeurs `export_method`
     attendues par `gym` — `app-store`/`ad-hoc`/…) ; retrait de l'appel `applivery()`
     de la lane `applivery_deploy` (la publication est désormais entièrement
     déléguée à l'action `emerald-mobile-publish-applivery`, qui uploade l'IPA déjà
     buildé sans repasser par Fastlane pour la partie AppLivery).

4. **Android — nouvelle lane Fastlane `:bundle`** (`androidApp/fastlane/Fastfile`) :
   génère un AAB (`clean bundle#{BUILD_VARIANT}`) pour les publications Store,
   symétrique à la lane `:assemble` existante (APK Qa).

## v8.5 (2026-07-06)

Corrections issues du premier run dispatch réel :

1. **matchRepos** : defaults `mobile-support/codesigning-ios-*` →
   **`Desjardins/mobile-codesigning-ios-*`** (miroirs GitHub réels des repos de
   certificats Match).

2. **`install-android-sdk: 'true'` explicite** sur les 3 invocations de
   `emerald-setup-android-env` (c'était déjà le défaut de l'action — rendu
   visible pour éviter toute désactivation accidentelle).

3. **Pourquoi « distribution: applivery » ne publiait pas** (diagnostic du run) :
   la chaîne dispatch fonctionnait (summary resolve-context : `distribution=
   applivery` ✓), mais les jobs 11 (Sonar androidApp) et 13 (Sonar iOS) étaient
   **ROUGES : `QUALITY GATE STATUS: FAILED`** → `gate-policy` met
   `allow_publish=false` → condition des jobs 40/42/43 fausse → **skip**. C'est
   la protection voulue : on ne publie pas un binaire dont la qualité est rouge.
   NB : `allowPublishWhenQualityGateSkipped` ne s'applique PAS ici (gate *failed*
   ≠ gate *skipped*).

   → **Nouvel input `forcePublish`** (workflow + dispatch consumer) : en run
   manuel, publie **même si `allow_publish=false`**. N'outrepasse ni le build
   vert requis ni les flags de distribution. Pour tester la mécanique de
   publication pendant que les quality gates Sonar sont encore rouges.
   Le vrai fix reste de faire passer les quality gates (dashboards liés dans le
   summary).

4. **`dryRun` documenté** : builds réels, jobs publish exécutés mais actions en
   mode simulation (log au lieu d'upload). `dryRun+forcePublish+distribution` =
   répétition générale complète sans rien envoyer aux stores.

5. **Liens Sonar du summary** : rappel — les clés ne sont PAS auto-détectées
   (elles vivent dans les build.gradle.kts, illisibles à ce stade). Renseigner
   `sharedSonarProjectKey` / `androidSonarProjectKey` / `iosSonarProjectKey` dans
   le consumer (valeurs exactes fournies dans le consumer-example). Le lien vu
   dans le step Android venait de l'ERREUR Gradle (le QG failed imprime l'URL) ;
   le job shared, vert, n'imprime rien — d'où son absence.
